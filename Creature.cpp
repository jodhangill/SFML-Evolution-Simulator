...
Creature Creature::reproduce()
{
	...
	if (s > 0 && e > 0.2) { // Efficiency cannot be too low
		Creature baby(/*sf::Vector2f(rand() % 1920 + 1, rand() % 1080 + 1)*/m_creature.getPosition(), s, e);
		return baby;
	}
	else {
		Creature baby(/*sf::Vector2f(rand() % 1920 + 1, rand() % 1080 + 1)*/m_creature.getPosition(), speed, eff);
		return baby;
	}
